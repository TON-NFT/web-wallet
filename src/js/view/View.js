...
class View {
  constructor(mnemonicWords) {
    ...
    $('#wordsConfirm_continueBtn').addEventListener('click', () => {
      ...
      if (!confirmWords.isWordsFromList) return
  ...
  createWordInputs(params) {
    ...
    const onPaste = (event) => {
      ...
      let arr = text.split(' ')
      if (arr.length !== params.count) arr = text.split(',')
    ...
    const item = createElement({
      ...
      child: [
        ...
        createElement({
          ...
          child: [
            isReceive
              ? createElement({ tag: 'span', clazz: 'tx-from', text: 'Received' })
              : createElement({ tag: 'span', clazz: 'tx-from', text: 'Sent' }),
            setAddr(createElement({ tag: 'div', clazz: ['tx-addr', 'addr'] }), formatAddress(addr, 8)),
          ]
        }),
        createElement({
          tag: 'div',
  ...
  setMyAddress(address) {
    setAddr($('.your-address'), formatAddress(address, 8))
    $('.your-address').addEventListener('click', () => copyToClipboard(address))
  ...
  onMessage(method, params) {
    switch (method) {
      ...
      case 'restoreDeprecatedStorage':
        ...
        return { address, words, walletVersion, proxy }
